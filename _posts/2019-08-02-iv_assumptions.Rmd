...
```{r, engine='tikz', echo=FALSE, fig.align='center',fig.width=400, cache=TRUE}
\begin{tikzpicture}
\node[] at (0,0) (iv) {IV};
\node[] at (2,0) (a) {A};
\node[] at (4,0) (y) {Y};
...
\draw[->] (c) -- (y);
\draw[->] (c) -- (y);
\draw[->, lightgray] (iv) to [out=330,in=210] node[] {$\times$} (y);
\draw[->, lightgray] (iv) -- node[] {$\times$} (c);
...
\draw[->] (c) -- (y);
\draw[->, lightgray] (iv) to [out=330,in=210] node[] {$\times$} (y);
\draw[->, lightgray] (c) -- node[] {$\times$} (iv);
\draw[->, red] (b) -- (iv);
\draw[->, red] (b) to [out=0,in=90] (y);
...
\node[] at (4,0) (y) {Y};
\node[] at (2,1) (c) {C};
\node[lightgray] at (0,1.5) (b) {B};
\node[lightgray] at (0,1.5) (x) {\LARGE{$\times$}};
\draw[->] (iv) -- (a);
\draw[->] (a) -- (y);
\draw[->] (c) -- (a);
\draw[->] (c) -- (y);
...
\draw[->, lightgray] (iv) to [out=330,in=210] node[] {$\times$} (y);
\draw[->, lightgray] (b) -- (iv);
\draw[->, lightgray] (b) to [out=0,in=90] (y);
